...
class MyDFS2:
    def __init__(self, name, max_bags=10, no_look=2, empty=2):
        self.NAME = name
        self.player = None
        self.MAPSIZE = 12
        self.POINT = 1.0
    ...
    def create_map(self, p1, p2, walls):
        ...
        self.delta_list = self.round_delta(self.NO_LOOK)

    def round_delta(self, k):
        round_list = list(range(-k, k+1))
        round_list = list(product(round_list, round_list))
        coord_list = list()
        for coord in round_list:
            if abs(coord[0]) + abs(coord[1]) <= k:
                coord_list.append(coord)
        coord_list.remove((0, 0))
        return coord_list

    def me_home_coord(self, me, home):
        x_lower = min(me[0], home[0])
        x_upper = max(me[0], home[0])
        y_lower = min(me[1], home[1])
        y_upper = max(me[1], home[1])
        x_range = list(range(x_lower, x_upper+1))
        y_range = list(range(y_lower, y_upper+1))
        coord_list = list(product(x_range, y_range))
        return coord_list
    ...
    def add_jobs(self, map, jobs, position):
        max_parcel = 0
        self.jobs_coords = list()
        for j in jobs:
            # map[j["x"], j["y"]] = j["value"]
            map[j["x"]][j["y"]] = j["value"]
            max_parcel = max(j["value"], max_parcel)
            self.jobs_coords.append((j["x"], j["y"], j["value"]))
    ...
    def step(self):
        if (self.pre_position[0], self.pre_position[1]) != (self.my_home[0], self.my_home[1]):
            a_star = AStar(Array2D(self.origin_map),
                           Point(self.pre_position[0], self.pre_position[1]), Point(self.my_home[0], self.my_home[1]),
                           passTag=-1)
            path_list = list(a_star.start())
        else:
            path_list = list()
        ...
        if not self.go_home and 10-self.n_jobs <= self.EMPTY:
            coord_list = self.me_home_coord(self.pre_position, self.my_home)
            jobs_list = list()
            for job in self.jobs_coords:
                if job[:2] in coord_list:
            ...
            if len(jobs_list) == 0:
                ...
                try:
                    self.pick_action = True
                    if len(jobs_list) == 1:
                        a_star = AStar(Array2D(self.origin_map),
                                       Point(self.pre_position[0], self.pre_position[1]),
                                       Point(jobs_list[0][0], jobs_list[0][1]),
                                       passTag=-1)
                        mp_path_list = list(a_star.start())
                        a_star = AStar(Array2D(self.origin_map),
                                       Point(jobs_list[0][0], jobs_list[0][1]),
                                       Point(self.my_home[0], self.my_home[1]),
                                       passTag=-1)
                        ph_path_list = list(a_star.start())
                        pick_path = mp_path_list + ph_path_list
                        # self.pick_path = list()
                        # for p in mp_path_list + ph_path_list:
                        #     self.pick_path.append((p.x, p.y))
                    else:
                        jobs_list = list(product(jobs_list, jobs_list))
                        min_len = 100
                        pick_path = None
                        for coords in jobs_list:
                            ...
                            a_star = AStar(Array2D(self.origin_map),
                                           ...
                                           passTag=-1)
                            pp_path_list = list(a_star.start())
                            a_star = AStar(Array2D(self.origin_map),
                                           Point(coords[1][0], coords[1][1]),
                                           Point(self.my_home[0], self.my_home[1]),
                                           passTag=-1)
                            ph_path_list = list(a_star.start())
                            if (coords[0][2]+coords[1][2])/float(len(mp_path_list) + len(ph_path_list) + len(pp_path_list)) < min_len:
                                pick_path = mp_path_list + pp_path_list + ph_path_list
                        # self.pick_path = list()
                        # for p in pick_path:
                        #     self.pick_path.append((p.x, p.y))
                except Exception:
        ...
        if self.go_home:
        ...
        elif not self.go_home and self.pick_action:
            print(self.NAME, "pick action")
            if not self.pick_path:
                self.pick_path = list()
                for p in pick_path:
                    self.pick_path.append((p.x, p.y))
            print(self.pick_path)
            print(self.pre_position)
            next_position = self.pick_path.pop(0)
            direction = self.move(next_position, self.pre_position)
            self.pre_position = next_position
            if not self.pick_path:
                self.pick_action = False
            ...
            for d in delta:
                self.explore(max_step, [self.pre_position[0]+d[0], self.pre_position[1]+d[1]],
                             self.help_map, self.real_map,
                            [self.pre_position[0]+d[0], self.pre_position[1]+d[1]], 0, 0, exploration, 1)
...
def self_battle():
    ...
    # sam bot1
    sam_bot1 = MyDFS2("p1", max_bags=10)

    # sam bot2
    sam_bot2 = MyDFS2("p2", max_bags=10, empty=0)
    p2_state = env_init
    ...
    for i in range(conf["max_steps"]):
        ...
        s = time.time()
        try:
            direction1 = sam_bot1.take_action(p2_state)
        except IndexError:
            print(sam_bot1.my_home)
            print(sam_bot1.pre_position)
            a = input()
        p1_time = time.time() - s
        p1_max_time = max(p1_time, p1_max_time)
        p1_state = env.step(sam_bot1.NAME, direction1)
        # sam_bot1.load_state(p1_state)

        # sam bot2 step
        s = time.time()
        try:
            direction2 = sam_bot2.take_action(p1_state)
        except IndexError:
            print(sam_bot2.my_home)
            print(sam_bot2.pre_position)
            a = input()
        p2_time = time.time() - s
        p2_max_time = max(p2_time, p2_max_time)
        p2_state = env.step(sam_bot2.NAME, direction2)
